"""Unit tests for the THD Selfcare RAG tools.

The tools talk to a fake `IndexerClient` injected through `register()`; no real
HTTP traffic. Covers: happy path per tool, error mapping, labels-cache TTL.
"""
...
from svc.mcp_telekom_thd_selfcare.indexer_client import IndexerError, LabelsCache
from svc.mcp_telekom_thd_selfcare.tools import register


class _FakeMCP:
    """Captures tools registered via `@mcp.tool(annotations=...)`."""
...
async def test_register_exposes_exactly_four_tools(fixture) -> None:
    ...
    assert set(tools.keys()) == {"list_documents", "search", "get_document", "list_labels"}
...
async def test_list_documents_returns_summary_and_pagination(fixture) -> None:
    ...
    result = json.loads(await tools["list_documents"](labels=["wifi"]))
    ...
    # index_id and labels passed through to client
...
async def test_search_passes_env_scoped_ids_and_returns_snippets(fixture) -> None:
    ...
    result = json.loads(
        await tools["search"](query="how to reset router", top_k=3, retrieval_mode="hybrid_bm25")
    )
...
async def test_get_document_fetches_doc_and_chunks_in_parallel(fixture) -> None:
    ...
    result = json.loads(await tools["get_document"](document_id=5))
...
async def test_list_labels_aggregates_across_pages_and_caches(fixture) -> None:
    ...
    first = json.loads(await tools["list_labels"]())
    ...
    second = json.loads(await tools["list_labels"]())
...
async def test_list_labels_refresh_busts_cache(fixture) -> None:
    ...
    second = json.loads(await tools["list_labels"](refresh=True))
    assert second["labels"] == ["a", "b"]


@pytest.mark.unit
async def test_search_maps_indexer_error_to_json_error(fixture) -> None:
    tools, client, _ = fixture
    client.search_responses.append(
        IndexerError("boom", status_code=503, body="service unavailable")
    )

    result = json.loads(await tools["search"](query="q"))

    assert result["error"] == "indexer_request_failed"
    assert result["status_code"] == 503
    assert "service unavailable" in result["indexer_response"]
